...
static esp_err_t calculate_adc_audio_info(esp_board_manager_adapter_info_t *info)
{
    ...
    /* 16 kHz / 16-bit is what the esp-sr AFE consumes natively (WakeNet, VAD and
     * NS all run at 16 kHz) and what the ES8389 coeff table supports
     * ({64, 1024000, 16000}). Reporting 48 kHz / 32-bit here made the recorder
     * push 6x the data the AFE expects, which overflowed the AFE FEED ring
     * buffer and wedged the afe_feed task on the task watchdog. */
...
esp_err_t esp_board_manager_adapter_init(const esp_board_manager_adapter_config_t *config, esp_board_manager_adapter_info_t *info)
{
    ...
    if (config->enable_audio) {
        /* Audio is treated as OPTIONAL: a missing/non-responsive codec must NOT
         * brick the whole application. The chat app already no-ops audio when
         * play_dev/rec_dev are NULL, so we continue and let the rest of the
         * board (LCD, touch, LVGL, network, AI connection) come up. */
        ret = esp_board_manager_get_device_handle("audio_dac", (void **)&basic_info.play_dev_handles);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "audio_dac handle unavailable (continuing without playback): %s", esp_err_to_name(ret));
            info->play_dev = NULL;
            basic_info.play_dev_handles = NULL;
        } else {
            ...
            if (info->play_dev) {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGW(TAG, "Failed to set playback volume (continuing): %s", esp_err_to_name(ret));
                } else {
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "audio_adc handle unavailable (continuing without recording): %s", esp_err_to_name(ret));
            info->rec_dev = NULL;
            basic_info.rec_dev_handles = NULL;
        } else {
            ...
            if (info->rec_dev) {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGW(TAG, "Failed to set recording gain (continuing): %s", esp_err_to_name(ret));
                } else {
        ...
        if (info->rec_dev != NULL) {
            ret = calculate_adc_audio_info(info);
            if (ret != ESP_OK) {
                ESP_LOGW(TAG, "Failed to calculate ADC audio info (continuing): %s", esp_err_to_name(ret));
            } else {
            ...
            ESP_LOGW(TAG, "Audio input unavailable; voice capture disabled but device continues");
            info->sample_rate = 16000;
            info->sample_bits = 16;
            info->channels = 1;
        }
