                                          ...
                                          size_t *out_len)
{
    ...
    esp_http_client_config_t config = {
        ...
        /* api.xiaozhi.me presents a leaf signed by "RapidSSL TLS RSA CA G1"
         * (DigiCert) but omits the intermediate in its TLS handshake, so the
         * stock ESP x509 crt verification cannot build a chain to a trusted
         * root ("No matching trusted root certificate found"). Supply the
         * exact intermediate + DigiCert Global Root G2 as the CA store. */
        .cert_pem = XIAOZHI_CA_CERT_PEM,
