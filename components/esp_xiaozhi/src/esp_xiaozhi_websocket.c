...
esp_err_t esp_xiaozhi_websocket_start(esp_xiaozhi_websocket_handle_t handle)
{
    ...
    /* Observability: log the WS endpoint (uri contains no credentials; token is
     * sent via header, not logged) so a TLS failure can be traced to the host. */
    ESP_LOGI(TAG, "WebSocket endpoint: %s (protocol v%d)", websocket->url, protocol_version);
    ...
    esp_websocket_client_config_t websocket_cfg = {
        ...
        /* MCP messages (e.g. self.camera.take_photo) are parsed and dispatched
         * on the websocket task. The default 4 KB stack overflows there:
         * esp_websocket_client's own frame handling + cJSON parsing + the MCP
         * tool callback trip esp_task_stack_is_sane_cache_disabled() when SPI
         * flash cache is toggled for NVS/cert access, so the board crashes the
         * moment an MCP tool is invoked. Give the task a real stack. */
        .task_stack = 12 * 1024,
        .task_prio = 8,
    };
