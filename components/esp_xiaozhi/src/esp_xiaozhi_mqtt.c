...
esp_err_t esp_xiaozhi_mqtt_start(esp_xiaozhi_mqtt_handle_t handle)
{
    ...
    /* Observability: log the broker host:port (no credentials) so a TLS
     * handshake failure can be correlated with the missing CA bundle entry. */
    ESP_LOGI(TAG, "MQTT broker endpoint: %s (port %d, transport SSL)", endpoint, ESP_XIAOZHI_MQTT_DEFAULT_PORT);
