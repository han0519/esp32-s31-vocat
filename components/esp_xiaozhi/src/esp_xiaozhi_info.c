...
esp_err_t esp_xiaozhi_chat_get_info(esp_xiaozhi_chat_info_t *info)
{
    ...
    do {
        ...
        ESP_LOGI(TAG, "OTA activation: POST %s (timeout %d ms)", CONFIG_XIAOZHI_OTA_URL, CONFIG_XIAOZHI_INFO_TIMEOUT_MS);
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "HTTP request to OTA server failed: %s", esp_err_to_name(ret));
            if (ret == ESP_ERR_HTTP_READ_TIMEOUT) {
                ESP_LOGE(TAG, "OTA server %s did not respond within %d ms. "
                              "TLS already succeeded (cert validated), so this is an HTTP-layer "
                              "timeout: check network reachability / firewall and that the device "
                              "is activated & bound at xiaozhi.me", CONFIG_XIAOZHI_OTA_URL, CONFIG_XIAOZHI_INFO_TIMEOUT_MS);
            }
            break;
        }
