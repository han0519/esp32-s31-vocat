...
static esp_err_t esp_video_init_sensor_and_video_device(const video_device_init_config_t *config)
{
    ...
    if (config->is_motor) {
        ...
        if (!cam_dev) {
            ...
            /* Patched by build_only.ps1: SCCB bus scan diagnostic.
             * Probe every 7-bit address on the shared I2C/SCCB bus so we can
             * tell whether the camera (0x68) is present at all, or whether the
             * bus only carries the codec (0x20) / touch (0x2A). */
            ESP_LOGW(TAG, "SCCB bus scan (port %d):", sccb_config->i2c_config.port);
            for (uint8_t a = 0x08; a < 0x78; a++) {
                if (i2c_master_probe(i2c_handle, a, 20) == ESP_OK) {
                    ESP_LOGW(TAG, "  responding device at 0x%02x", a);
                }
            }
            goto fail_2;
        }
