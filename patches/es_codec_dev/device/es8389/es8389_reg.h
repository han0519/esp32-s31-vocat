...
/* VoCat-S31 wires its mic single-ended on MIC1P (Left) / MIC2P (Right), NOT as
 * the differential MIC1P-MIC1N pair. The upstream differential default made the
 * PGA common-mode-reject the mic signal, so the ADC only ever saw silence and
 * WakeNet never fired. Use single-ended MIC1P for the Left (mic) channel. */
#define ES8389_ADCL_InputSel  ES8389_ADC_INSEL_L_SINGLE_MIC1P
