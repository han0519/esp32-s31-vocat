...
// Patched for ESP-VoCat-S31 (喵伴) — write retry to ride out flaky I2C writes (LDO settling).
#define ES8389_WRITE_RETRY_TIMES 5
#define ES8389_WRITE_RETRY_DELAY_MS 30

static int es8389_write_reg(audio_codec_es8389_t *codec, int reg, int value)
{
    int ret = -1;
    for (int t = 0; t < ES8389_WRITE_RETRY_TIMES; t++) {
        ret = codec->cfg.ctrl_if->write_reg(codec->cfg.ctrl_if, reg, 1, &value, 1);
        if (ret == 0) {
            return 0;
        }
        ESP_LOGW(TAG, "write reg 0x%02x=0x%02x failed (%d), retry %d/%d", reg, value, ret, t + 1, ES8389_WRITE_RETRY_TIMES);
        vTaskDelay(pdMS_TO_TICKS(ES8389_WRITE_RETRY_DELAY_MS));
    }
    return ret;
}
...
static int es8389_open(const audio_codec_if_t *h, void *cfg, int cfg_size)
{
    ...
    // Patched for ESP-VoCat-S31 (喵伴) — give the CODEC_3V3 LDO a beat to settle
    // before kicking off the I2C burst. Without this delay the first few writes
    // often fail (the I2C bus sees the chip power up partway through init).
    ...
    /* VoCat-S31 uses electret (biased) mics wired single-ended on MIC1P/MIC2P.
     * 0x62 is the MICBIAS control: bits[2:0] select the bias *voltage* and bit7
     * is the MICBIAS power bit. The empirical mic sweep proved 0x80 (power up,
     * voltage bits = 0) leaves the electret starved -> the ADC only ever captured
     * digital silence and WakeNet never fired. A non-zero bias voltage is required:
     * 0x87 = power up + max voltage (v=7) gives the strongest, cleanest mic level. */
    ret |= es8389_write_reg(codec, ES8389_ANALOG_CONTROL_REG0x62, 0x87);
