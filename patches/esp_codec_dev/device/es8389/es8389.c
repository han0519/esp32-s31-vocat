...
static int es8389_open(const audio_codec_if_t *h, void *cfg, int cfg_size)
{
    ...
    /* VoCat-S31 uses electret (biased) mics on the differential MIC1/MIC2 inputs.
     * Bit7 of 0x62 powers the MICBIAS LDO; with it clear (the upstream default 0x4F)
     * the mics receive no bias and the ADC only ever captures digital silence. */
